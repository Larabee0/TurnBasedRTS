...
namespace DOTSHexagons
{
    public class HexMapCamera : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            gridData = entityManager.GetComponentData<HexGridComponent>(HexMetrics.ActiveGridEntity);
            if (!cameraStarted)
            {
                ValidatePosition();
                cameraStarted = true;
        ...
        {
            float xMax = (gridData.cellCountX - 0.5f) * HexMetrics.innerDiameter;
            float zMax = (gridData.cellCountZ - 1f) * (1.5f * HexMetrics.outerRadius);
        ...
        {
            float width = gridData.cellCountX * HexMetrics.innerDiameter;
            while (position.x < 0f)
            {
                position.x += width;
            }
            ...
            float zMax = (gridData.cellCountZ - 1) * (1.5f * HexMetrics.outerRadius);

            position.z = math.clamp(position.z, 0f, zMax);

            entityManager.AddComponentData(HexMetrics.ActiveGridEntity, new CentreMap { value = position.x });
            return position;
        }

        private void AdjustZoom(float delta)
        {
            zoom = HexMetrics.Clamp01(zoom + delta);
