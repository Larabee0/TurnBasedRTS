...
namespace DOTSHexagons
{
	...
	public struct TriangulatorEverything : IJobEntityBatch
	{
		...
		[ReadOnly]
		public NativeHashMap<int2, int> chunkProcessingIndice;
		[ReadOnly]
		...
		[ReadOnly]
		public BufferFromEntity<Child> childBufferType;
		[ReadOnly]
		...
		public void Execute(ArchetypeChunk batchInChunk, int batchIndex)
		{
			...
			for (int i = 0; i < hGCC.Length; i++)
			{
				...
				int meshStart = chunkProcessingIndice[new int2(chunkComp.chunkIndex, chunkComp.gridEntity.Index)] * 7;
				...
				ecbBegin.SetBuffer<Float3ForCollider>(batchIndex, chunkComp.entityTerrian).CopyFrom(terrianMesh.vertices.AsArray().Reinterpret<Float3ForCollider>());
				ecbBegin.SetBuffer<UintForCollider>(batchIndex, chunkComp.entityTerrian).CopyFrom(terrianMesh.triangles.AsArray().Reinterpret<UintForCollider>());
				ecbBegin.AddComponent<SetCollider>(batchIndex, chunkComp.entityTerrian);

				terrianMesh.ApplyMesh(meshDataArray[meshStart]);
				waterMesh.ApplyMesh(meshDataArray[meshStart + 2]);
				riverMesh.ApplyMesh(meshDataArray[meshStart + 1]);
				roadMesh.ApplyMesh(meshDataArray[meshStart + 5]);
				waterShoreMesh.ApplyMesh(meshDataArray[meshStart + 3]);
				estuaryMesh.ApplyMesh(meshDataArray[meshStart + 4]);
				wallMesh.ApplyMesh(meshDataArray[meshStart + 6]);
				...
				DynamicBuffer<Child> cellContainers = childBufferType[chunkComp.FeatureContainer];
		...
		{
			float3 c1 = centre + HexMetrics.GetFirstWaterCorner(direction);
			float3 c2 = centre + HexMetrics.GetSecondWaterCorner(direction);
								...
								case true:
									...
									switch (nextNeighbor.IsUnderwater)
									{
										case true:
											...
											AddTriangleInfo(wrapSize, waterMesh, c2, e2, c2 + HexMetrics.GetWaterBridge(direction.Next()), indices, weights1, weights2, weights3);
											break;
									}
									break;
							}
		...
		private void TriangulateWaterShore (DynamicBuffer<HexCell> cells, MeshData waterMesh, MeshUV waterShoreMesh, Mesh2UV estuariesMesh, HexDirection direction, HexCell cell, HexCell neighbour, float3 centre)
		{
			...
			{
				case true:
					centre2.x += cell.wrapSize * HexMetrics.innerDiameter;
					break;
				case false:
					switch (neighbour.ColumnIndex > cell.ColumnIndex + 1)
					{
			...
			switch (nextNeighborIndex != int.MinValue)
			{
				case true:
					...
					{
						case true:
							centre3.x += cell.wrapSize * HexMetrics.innerDiameter;
							break;
						case false:
							switch (nextNeighbor.ColumnIndex > cell.ColumnIndex + 1)
							{
					...
					float3 v3 = centre3 + (nextNeighbor.IsUnderwater ? HexMetrics.GetFirstWaterCorner(direction.Previous()) : HexMetrics.GetFirstSolidCorner(direction.Previous()));
					v3.y = centre.y;
		...
		private void TriangulateConnection(DynamicBuffer<HexCell> cells, MeshData terrianMesh, MeshUV riverMesh, MeshUV roadMesh, MeshBasic wallMesh, NativeList<PossibleFeaturePosition> features, HexDirection direction, HexCell cell, EdgeVertices e1)
		{
			...
			float3 bridge = HexMetrics.GetBridge(direction);
			...
			{
				case true:
					float3 v5 = e1.v5 + HexMetrics.GetBridge(direction.Next());
		...
		private void TriangulateCornerCliffTerraces(MeshData terrianMesh, float3 begin, HexCell beginCell, float3 left, HexCell leftCell, float3 right, HexCell rightCell)
		{
			...
			float3 boundary = math.lerp(HexMetrics.Perturb(noiseColours, begin, beginCell.wrapSize), HexMetrics.Perturb(noiseColours, left, beginCell.wrapSize), b);
		...
		{
			float3 v2 = HexMetrics.Perturb(noiseColours, HexMetrics.TerraceLerp(begin, left, 1), wrapSize);
			float4 w2 = HexMetrics.TerraceLerp(beginWeights, leftWeights, 1);

			AddTriangleInfoUnperturbed(terrianMesh, HexMetrics.Perturb(noiseColours, begin, wrapSize), v2, boundary, indices, beginWeights, w2, boundaryWeights);
			for (int i = 2; i < HexMetrics.terraceSteps; i++)
			{
				float3 v1 = v2;
				float4 w1 = w2;
				v2 = HexMetrics.Perturb(noiseColours, HexMetrics.TerraceLerp(begin, left, i), wrapSize);
			...
			AddTriangleInfoUnperturbed(terrianMesh, v2, HexMetrics.Perturb(noiseColours, left, wrapSize), boundary, indices, w2, leftWeights, boundaryWeights);
		}
		...
		private void TriangulateCornerTerraces(MeshData terrianMesh, float3 begin, HexCell beginCell, float3 left, HexCell leftCell, float3 right, HexCell rightCell)
		{
			...
			float4 w3 = HexMetrics.TerraceLerp(weights1, weights2, 1);
			float4 w4 = HexMetrics.TerraceLerp(weights1, weights3, 1);
			float3 indices = new float3(beginCell.Index, leftCell.Index, rightCell.Index);
		...
		private void TriangulateWaterfallInWater (int wrapSize,MeshUV riverMesh, float3 v1, float3 v2, float3 v3, float3 v4, float y1, float y2, float waterY, float3 indices)
		{
			v1.y = v2.y = y1;
			v3.y = v4.y = y2;
			v1 = HexMetrics.Perturb(noiseColours, v1, wrapSize);
			v2 = HexMetrics.Perturb(noiseColours, v2, wrapSize);
			v3 = HexMetrics.Perturb(noiseColours, v3, wrapSize);
			v4 = HexMetrics.Perturb(noiseColours, v4, wrapSize);
										...
										case false:
											switch (previousHasRiver && nextHasRiver)
											{
												...
												case false:
													...
													float3 offsetOther = HexMetrics.GetSolidEdgeMiddle(middle);
													roadCentre += offsetOther * 0.25f;
													switch (direction == middle && HexCell.HasRoadThroughEdge(cell, direction.Opposite()))
													{
														case true:
															AddFeature(features, cell.Index, roadCentre, centre - offsetOther * (HexMetrics.innerToOuter * 0.7f), FeatureCollection.Bridge);
															break;
													}
													break;
											}
		...
		private void AddTriangleInfo (int wrapSize, MeshData mesh, float3 v1, float3 v2, float3 v3, float3 indices, float4 weights1, float4 weights2, float4 weights3)
		{
			...
			mesh.verticesInternalTri[1] = HexMetrics.Perturb(noiseColours, v2, wrapSize);
			mesh.verticesInternalTri[2] = HexMetrics.Perturb(noiseColours, v3, wrapSize);
		...
		{
			uint vertexIndex = mesh.VertexIndex;
			mesh.verticesInternalQuad[0] = HexMetrics.Perturb(noiseColours, v1, wrapSize);
			mesh.verticesInternalQuad[1] = HexMetrics.Perturb(noiseColours, v2, wrapSize);
			mesh.verticesInternalQuad[2] = HexMetrics.Perturb(noiseColours, v3, wrapSize);
			mesh.verticesInternalQuad[3] = HexMetrics.Perturb(noiseColours, v4, wrapSize);
		...
		private void AddWallSegment(MeshBasic wallMesh, NativeList<PossibleFeaturePosition> features, int hexGridCell, float3 pivot, HexCell pivotCell, float3 left, HexCell leftCell, float3 right, HexCell rightCell, int towerIndex = 0)
		{
			...
			if (hasLeftWall)
			{
				if (hasRightWall)
				{
					...
					{
						HexHash hash = HexMetrics.SampleHashGrid(hexHashData[pivotCell.grid].AsNativeArray(), (pivot + left + right) * (1f / 3f));
		...
		{
			nearLeft = HexMetrics.Perturb(noiseColours, nearLeft, wrapSize);
			farLeft = HexMetrics.Perturb(noiseColours, farLeft, wrapSize);
			nearRight = HexMetrics.Perturb(noiseColours, nearRight, wrapSize);
			farRight = HexMetrics.Perturb(noiseColours, farRight, wrapSize);
		...
		{
			near = HexMetrics.Perturb(noiseColours, near, wrapSize);
			far = HexMetrics.Perturb(noiseColours, far, wrapSize);
			point = HexMetrics.Perturb(noiseColours, point, wrapSize);

			float3 centre = HexMetrics.WallLerp(near, far);
		...
		private void WallsAddTriangleInfoUnperturbed(int wrapSize, MeshBasic wallMesh, float3 v1, float3 v2, float3 v3)
		{
			...
			wallMesh.vertices[0] = HexMetrics.Perturb(noiseColours, v1, wrapSize);
			wallMesh.vertices[1] = HexMetrics.Perturb(noiseColours, v2, wrapSize);
			wallMesh.vertices[2] = HexMetrics.Perturb(noiseColours, v3, wrapSize);
		...
		private struct MeshUV : INativeDisposable
		{
			...
			public void ApplyMesh(Mesh.MeshData meshData)
			{
				...
				meshData.GetVertexData<float4>(1).CopyFrom(cellWeights);
				meshData.GetVertexData<float3>(2).CopyFrom(cellIndices);
				meshData.GetVertexData<float2>(3).CopyFrom(uvs);
				meshData.GetIndexData<uint>().CopyFrom(triangles);
				meshData.subMeshCount = 1;
				meshData.SetSubMesh(0, new SubMeshDescriptor(0, triangles.Length, MeshTopology.Triangles));
			}
		...
		private struct Mesh2UV : INativeDisposable
		{
			...
			public void ApplyMesh(Mesh.MeshData meshData)
			{
				...
				meshData.GetVertexData<float4>(1).CopyFrom(cellWeights);
				meshData.GetVertexData<float3>(2).CopyFrom(cellIndices);
				meshData.GetVertexData<float4>(3).CopyFrom(uvs);
				meshData.GetIndexData<uint>().CopyFrom(triangles);
				meshData.subMeshCount = 1;
				meshData.SetSubMesh(0, new SubMeshDescriptor(0, triangles.Length, MeshTopology.Triangles));
			}
