...
public partial struct TriangulateChunksJob : IJobEntity
{
    ...
    public void Execute([ChunkIndexInQuery] int jobChunkIndex, Entity chunkEntity, ref DynamicBuffer<HexFeatureRequest> featureRequests,ref DynamicBuffer<HexChunkCellWrapper> wrappedCells, in DynamicBuffer<HexCellReference> chunkCells, in HexChunkTag chunkTag, in HexChunkMeshEntities meshEntities)
    {
        int terrainIndex = chunkToMeshMap[new int2(chunkTag.Index, 0)];
        int riversIndex = chunkToMeshMap[new int2(chunkTag.Index, 1)];
        int waterIndex = chunkToMeshMap[new int2(chunkTag.Index, 2)];
        int waterShoreIndex = chunkToMeshMap[new int2(chunkTag.Index, 3)];
        int estuariesIndex = chunkToMeshMap[new int2(chunkTag.Index, 4)];
        int roadsIndex = chunkToMeshMap[new int2(chunkTag.Index, 5)];
        int wallsIndex = chunkToMeshMap[new int2(chunkTag.Index, 6)];
        NativeArray<HexChunkCellWrapper> allChunkCells = wrappedCells.AsNativeArray();
        allChunkCells.Sort(new WrappedCellIndexSorter());

        MeshWrapper meshWrapper = new(noiseColours, wrapSize);

        for (int c = 0; c < chunkCells.Length; c++)
        {
            int cellIndex = allChunkCells.BinarySearch(new HexChunkCellWrapper { cellBasic = new HexCellBasic { Index = chunkCells[c].Index } });
            if (cellIndex != -1)
            {
                HexChunkCellWrapper cell = allChunkCells[cellIndex];
                
                for (HexDirection d = HexDirection.NE; d <= HexDirection.NW; d++)
                {
                    Triangulate(meshWrapper, allChunkCells, d, cell);
                }
        ...
        meshWrapper.terrianMesh.ApplyMesh(meshDataArray[terrainIndex]);
        meshWrapper.riverMesh.ApplyMesh(meshDataArray[riversIndex]);
        meshWrapper.waterMesh.ApplyMesh(meshDataArray[waterIndex]);
        meshWrapper.waterShoreMesh.ApplyMesh(meshDataArray[waterShoreIndex]);
        meshWrapper.estuaryMesh.ApplyMesh(meshDataArray[estuariesIndex]);
        meshWrapper.roadMesh.ApplyMesh(meshDataArray[roadsIndex]);
        meshWrapper.wallMesh.ApplyMesh(meshDataArray[wallsIndex]);
        ecbEnd.AddComponent(jobChunkIndex, meshEntities.Terrain, new HexMeshChunkIndex { meshArrayIndex = terrainIndex });
        ecbEnd.AddComponent(jobChunkIndex, meshEntities.Rivers, new HexMeshChunkIndex { meshArrayIndex = riversIndex });
        ecbEnd.AddComponent(jobChunkIndex, meshEntities.Water, new HexMeshChunkIndex { meshArrayIndex = waterIndex });
        ecbEnd.AddComponent(jobChunkIndex, meshEntities.WaterShore, new HexMeshChunkIndex { meshArrayIndex = waterShoreIndex });
        ecbEnd.AddComponent(jobChunkIndex, meshEntities.Estuaries, new HexMeshChunkIndex { meshArrayIndex = estuariesIndex });
        ecbEnd.AddComponent(jobChunkIndex, meshEntities.Roads, new HexMeshChunkIndex { meshArrayIndex = roadsIndex });
        ecbEnd.AddComponent(jobChunkIndex, meshEntities.Walls, new HexMeshChunkIndex { meshArrayIndex = wallsIndex });
        ...
        if (featureRequests.Length > 0 || meshWrapper.featureRequests.Length > 0)
        {
            if (meshWrapper.featureRequests.Length == 0)
            {
                // clear features
                featureRequests.Clear();
                ecbEnd.AddComponent<HexFeatureUpdate>(jobChunkIndex, chunkEntity);
            }
            else if (meshWrapper.featureRequests.Length == featureRequests.Length)
            {
                NativeParallelHashSet<HexFeatureRequest> featureRequestSet = new(featureRequests.Length, Allocator.Temp);
                for (int i = 0; i < featureRequests.Length; i++)
                {
                    featureRequestSet.Add(featureRequests[i]);
                }
                bool changed = false;
                for (int i = 0; i < meshWrapper.featureRequests.Length; i++)
                {
                    if (!featureRequestSet.Contains(meshWrapper.featureRequests[i]))
                    {
                        changed = true;
                        break;
                    }
                ...
                {
                    featureRequests.Clear();
                    featureRequests.AddRange(meshWrapper.featureRequests.AsArray());
                    ecbEnd.AddComponent<HexFeatureUpdate>(jobChunkIndex, chunkEntity);
                }
            }
            else
    ...
    private void Triangulate(MeshWrapper wrapper, NativeArray<HexChunkCellWrapper> allChunkCells,HexDirection d, HexChunkCellWrapper cell)
    {
        ...
        {
            TriangulateConnection(wrapper, allChunkCells, d, cell, e);
        }

        if (cell.cellTerrain.IsUnderwater)
        {
            TriangulateWater(wrapper, allChunkCells, d, cell, center);
        }
    }

    void TriangulateWater(MeshWrapper wrapper, NativeArray<HexChunkCellWrapper> allChunkCells,
        HexDirection d, HexChunkCellWrapper cell, float3 center
    )
    {
        center.y = cell.cellTerrain.WaterSurfaceY;

        int neighbourIndex = GetNeighbourIndex(allChunkCells, d, cell);
        if (neighbourIndex < 0)
        {
            TriangulateOpenWater(wrapper, allChunkCells, d, cell, neighbourIndex, center);
            return;
        }

        HexChunkCellWrapper neighbor = allChunkCells[neighbourIndex];
        if (!neighbor.cellTerrain.IsUnderwater)
        {
            TriangulateWaterShore(wrapper,allChunkCells,d, cell, neighbor, center);
        }
        else
        {
            TriangulateOpenWater(wrapper, allChunkCells, d, cell, neighbourIndex, center);
    ...
    private static int GetNeighbourIndex(NativeArray<HexChunkCellWrapper> allChunkCells, HexDirection d, HexChunkCellWrapper cell)
    {
        return allChunkCells.BinarySearch(new HexChunkCellWrapper { cellBasic = new HexCellBasic { Index = cell.GetNeighbourIndex(d) } });
    }

    private void TriangulateOpenWater(MeshWrapper wrapper, NativeArray<HexChunkCellWrapper> allChunkCells,
        HexDirection direction, HexChunkCellWrapper cell, int neighbourIndex, float3 center
    )
    {
        ...
        {
            HexChunkCellWrapper neighbour = allChunkCells[neighbourIndex];
            ...
            {
                int nextNeighbourIndex = GetNeighbourIndex(allChunkCells, direction.Next(), cell);
                if (nextNeighbourIndex > 0)
                {
                    HexChunkCellWrapper nextNeighbour = allChunkCells[nextNeighbourIndex];
    ...
    void TriangulateWaterShore(MeshWrapper wrapper,NativeArray<HexChunkCellWrapper> allChunkCells,
        HexDirection direction, HexChunkCellWrapper cell, HexChunkCellWrapper neighbor, float3 center
    )
    {
        ...
        {
            TriangulateEstuary(wrapper,
                e1, e2,
                cell.cellTerrain.hasIncomingRiver && cell.cellTerrain.IncomingRiver == direction, indices
            );
        }
        else
        {
        ...
        int nextNeighborIndex = GetNeighbourIndex(allChunkCells, direction.Next(), cell);

        
        if (nextNeighborIndex >=  0)
        {
            HexChunkCellWrapper nextNeighbor = allChunkCells[nextNeighborIndex];
            ...
            float3 v3 = center3 + (nextNeighbor.cellTerrain.IsUnderwater ?
    ...
    void TriangulateEstuary(MeshWrapper wrapper,
        EdgeVertices e1, EdgeVertices e2, bool incomingRiver, float3 indices
    )
    {
    ...
    private void TriangulateConnection(MeshWrapper wrapper, NativeArray<HexChunkCellWrapper> allChunkCells, HexDirection d, HexChunkCellWrapper cell, EdgeVertices e1)
    {
        int neighbourIndex = GetNeighbourIndex(allChunkCells,d,cell);
        if (neighbourIndex < 0)
        {
        ...
        if (hasRiver)
        {
            ...
            if (!cell.cellTerrain.IsUnderwater)
            {
                ...
                !neighbour.cellTerrain.IsUnderwater &&
                neighbour.Elevation > cell.cellTerrain.waterLevel
            )
            {
                TriangulateWaterfallInWater(wrapper,
        ...
        int nextNeighbourIndex = GetNeighbourIndex(allChunkCells, d.Next(), cell);
    ...
    private void TriangulateCorner(MeshWrapper wrapper,
        float3 bottom, HexChunkCellWrapper bottomCell,
        float3 left, HexChunkCellWrapper leftCell,
        float3 right, HexChunkCellWrapper rightCell)
    {
    ...
    private void TriangulateBoundaryTriangle(MeshWrapper wrapper, float3 begin,float4 beginWeights,
        float3 left, float4 leftWeights, float3 boundary,float4 boundaryWeights,float3 indices)
    {
    ...
    void TriangulateWithoutRiver(MeshWrapper wrapper,
        HexDirection direction, HexChunkCellWrapper cell, float3 center, EdgeVertices e
    )
    {
        ...
        if (cell.cellTerrain.HasRoads)
        {
            ...
            TriangulateRoad(wrapper,
                center,
                math.lerp(center, e.c0, interpolators.x),
                math.lerp(center, e.c4, interpolators.y),
                e, cell.cellTerrain.HasRoadThroughEdge(direction), cell.Index
            );
        }
    }

    void TriangulateWithRiver(MeshWrapper wrapper,
        HexDirection direction, HexChunkCellWrapper cell, float3 center, EdgeVertices e
    )
    {
        float3 centerL, centerR;
        ...
        if (!cell.cellTerrain.IsUnderwater)
        {
            ...
            TriangulateRiverQuad(wrapper,
                centerL, centerR, m.c1, m.c3,
                cell.cellTerrain.RiverSurfaceY, cell.cellTerrain.RiverSurfaceY, 0.4f, reversed, indices
            );
            TriangulateRiverQuad(wrapper,
                m.c1, m.c3, e.c1, e.c3,
                cell.cellTerrain.RiverSurfaceY, cell.cellTerrain.RiverSurfaceY, 0.6f, reversed, indices
            );
        }
    }
    void TriangulateAdjacentToRiver(MeshWrapper wrapper,
        HexDirection direction, HexChunkCellWrapper cell, float3 center, EdgeVertices e
    )
    {
        if (cell.cellTerrain.HasRoads)
        ...
        if (cell.cellTerrain.HasRiverThroughEdge(direction.Next()))
        {
            if (cell.cellTerrain.HasRiverThroughEdge(direction.Previous()))
            {
                ...
                cell.cellTerrain.HasRiverThroughEdge(direction.Previous2())
            )
            {
    ...
    void TriangulateRoadAdjacentToRiver(MeshWrapper wrapper,
        HexDirection direction, HexChunkCellWrapper cell, float3 center, EdgeVertices e
    )
    {
        ...
        if (cell.cellTerrain.HasRiverBeginOrEnd)
        {
            roadCenter += HexMetrics.GetSolidEdgeMiddle(
                cell.cellTerrain.RiverBeginOrEndDirection.Opposite()
            ) * (1f / 3f);
        }
        else if (cell.cellTerrain.IncomingRiver == cell.cellTerrain.OutgoingRiver.Opposite())
        {
            ...
            if (previousHasRiver)
            {
                if (
                    !hasRoadThroughEdge &&
                    !cell.cellTerrain.HasRoadThroughEdge(direction.Next())
                )
                {
                    return;
                }
            ...
            else
            {
                if (
                    !hasRoadThroughEdge &&
                    !cell.cellTerrain.HasRoadThroughEdge(direction.Previous())
                )
                {
                    return;
                }
        ...
        else if (previousHasRiver && nextHasRiver)
        {
            ...
            float3 offset = HexMetrics.GetSolidEdgeMiddle(direction) *
        ...
        else
        {
            ...
            if (
                !cell.cellTerrain.HasRoadThroughEdge(middle) &&
    ...
    void TriangulateWaterfallInWater(MeshWrapper wrapper, float3 v1, float3 v2, float3 v3, float3 v4, float y1, float y2, float waterY, float3 indices)
    {
        v1.y = v2.y = y1;
        v3.y = v4.y = y2;
        v1 = Perturb(v1);
    ...
    void TriangulateRoad(MeshWrapper wrapper,
        float3 center, float3 mL, float3 mR,
    ...
    )
    {
        if (hasRoadThroughCellEdge)
        {
            ...
            TriangulateRoadSegment(wrapper,
                mL, mC, mR, e.c1, e.c2, e.c3,
    ...
    void TriangulateRoadEdge(MeshWrapper wrapper,
        float3 center, float3 mL, float3 mR, float index
    )
    {
    ...
    private float2 GetRoadInterpolators(HexDirection direction, HexChunkCellWrapper cell)
    {
        ...
        {
            interpolators.x =
                cell.cellTerrain.HasRoadThroughEdge(direction.Previous()) ? 0.5f : 0.25f;
            interpolators.y =
                cell.cellTerrain.HasRoadThroughEdge(direction.Next()) ? 0.5f : 0.25f;
        }
        return interpolators;
    }
    void TriangulateWithRiverBeginOrEnd(MeshWrapper wrapper,HexChunkCellWrapper cell, float3 center, EdgeVertices e)
    {
        EdgeVertices m = new(math.lerp(center, e.c0, 0.5f), math.lerp(center, e.c4, 0.5f));
        m.c2.y = e.c2.y;
    ...
    private void AddWall(MeshWrapper wrapper,
        EdgeVertices near, HexChunkCellWrapper nearCell,
        EdgeVertices far, HexChunkCellWrapper farCell,
        bool hasRiver, bool hasRoad
    )
    {
        if (
            nearCell.cellTerrain.walled != farCell.cellTerrain.walled &&
    ...
    private void AddWall(MeshWrapper wrapper,
        float3 c0, HexChunkCellWrapper cell0,
        float3 c1, HexChunkCellWrapper cell1,
        float3 c2, HexChunkCellWrapper cell2)
    {
    ...
    void AddWallSegment(MeshWrapper wrapper,
        float3 nearLeft, float3 farLeft, float3 nearRight, float3 farRight,
        bool addTower = false
    )
    {
        ...
        float3 leftThicknessOffset =
            HexMetrics.WallThicknessOffset(nearLeft, farLeft);
        float3 rightThicknessOffset =
    ...
    void AddWallSegment(MeshWrapper wrapper,
        float3 pivot, HexChunkCellWrapper pivotCell,
        float3 left, HexChunkCellWrapper leftCell,
        float3 right, HexChunkCellWrapper rightCell
    )
    {
        ...
        if (hasLeftWall)
        {
            if (hasRighWall)
            {
                ...
                if (leftCell.Elevation == rightCell.Elevation)
                {
                    HexHash hash = HexMetrics.SampleHashGrid(hashGrid,
    ...
    public void AddSpecialFeature(MeshWrapper wrapper, HexChunkCellWrapper cell, float3 position)
    {
        HexHash hash = HexMetrics.SampleHashGrid(hashGrid, position);
        wrapper.featureRequests.Add(new HexFeatureRequest
        {
            type = HexFeatureType.Generic,
            prefab = specialPrefabs[cell.cellTerrain.specialIndex - 1].prefab,
