...
namespace Unity.Physics
{
    ...
    internal static class MeshBuilder
    {
        ...
        internal static unsafe TempSection BuildSections(BoundingVolumeHierarchy.Node* nodes, int nodeCount, NativeList<MeshConnectivityBuilder.Primitive> primitives)
        {
            ...
            do
            {
                ...
                while (subTreeIndices.Length > 0)
                {
                    ...
                    var vertexIndices = new NativeArray<int3>(tmpVertices.Length/3, Allocator.Temp);
                    for (int i = 0, v = 0; i < vertexIndices.Length; i++,v+=3)
                    {
                        vertexIndices[i] = new int3(v, v + 1, v + 2);
                    }
        ...
        internal static unsafe TempSection BuildSectionsManaged(BoundingVolumeHierarchy.Node* nodes, int nodeCount, NativeList<MeshConnectivityBuilder.Primitive> primitives)
        {
            var tempSections = new TempSection()
            {
                PrimitivesFlags = new NativeList<Mesh.PrimitiveFlags>(Allocator.Persistent),
                Primitives = new NativeList<Mesh.PrimitiveVertexIndices>(Allocator.Persistent),
                Vertices = new NativeList<float3>(Allocator.Persistent),
                Ranges = new NativeList<TempSectionRanges>(Allocator.Persistent)
            };
            ...
            var primitivesCountInSubTree = ProducedPrimitivesCountPerSubTreeManaged(nodes, nodeCount);

            var subTreeIndices = new NativeList<int>(Allocator.Persistent);
            var nodeIndices = new NativeList<int>(Allocator.Persistent);
            var tmpVertices = new NativeList<float3>(Allocator.Persistent);
            do
            {
                int nodeIndex = nodesIndexStack[--stackSize];
                int subTreeVertexCountEstimate = (int)(uniqueVerticesPerPrimitiveFactor * primitivesCountInSubTree[nodeIndex]);
                ...
                while (subTreeIndices.Length > 0)
                {
                    ...
                    NativeArray<float3> verticesForWelder = new(tmpVertices.AsArray(), Allocator.Persistent);
                    var vertexIndices = new NativeArray<int3>(tmpVertices.Length / 3, Allocator.Persistent);
                    for (int i = 0, v = 0; i < vertexIndices.Length; i++, v += 3)
                    {
                        vertexIndices[i] = new int3(v, v + 1, v + 2);
                    }

                    NativeList<float3> uniqueVertices = MeshConnectivityBuilder.WeldVerticesManaged(vertexIndices, verticesForWelder);
                    verticesForWelder.Dispose();
                    vertexIndices.Dispose();
                    if (uniqueVertices.Length < Mesh.Section.MaxNumVertices)
                    {
                        verticesForWelder = new NativeArray<float3>(uniqueVertices.AsArray(), Allocator.Persistent);
                        BuildSectionGeometry(tempSections, primitives, nodeIndices, nodes, verticesForWelder);
                        verticesForWelder.Dispose();
                        // Remove used indices
                        for (var i = 0; i < nodeIndices.Length; ++i)
                        {
            ...
            while (stackSize > 0);
            primitivesCountInSubTree.Dispose();
            subTreeIndices.Dispose();
            nodeIndices.Dispose();
            tmpVertices.Dispose();
            return tempSections;
        }
        ...
        private static unsafe NativeArray<int> ProducedPrimitivesCountPerSubTreeManaged(BoundingVolumeHierarchy.Node* nodes, int nodeCount)
        {
            var primitivesPerNode = new NativeArray<int>(nodeCount, Allocator.Persistent);
    ...
    internal struct MeshConnectivityBuilder
    {
        ...
        public static NativeList<float3> WeldVertices(NativeArray<int3> indices, NativeArray<float3> vertices)
        {
        ...
        public static NativeList<float3> WeldVerticesManaged(NativeArray<int3> indices, NativeArray<float3> vertices)
        {
            int numVertices = vertices.Length;
            var verticesAndHashes = new NativeArray<VertexWithHash>(numVertices, Allocator.Persistent);
            for (int i = 0; i < numVertices; i++)
            {
                verticesAndHashes[i] = new VertexWithHash()
                {
            ...
            var uniqueVertices = new NativeList<float3>(Allocator.Persistent);
            var remap = new NativeArray<int>(numVertices, Allocator.Persistent);
            verticesAndHashes.Sort(new SortVertexWithHashByHash());
        ...
        internal unsafe MeshConnectivityBuilder(NativeArray<int3> triangles, NativeArray<float3> vertices,Allocator allocator)
        {
            int numTriangles = triangles.Length;
            int numVertices = vertices.Length;

            Vertices = new NativeArray<Vertex>(numVertices, allocator);
            ...
            // Compute edges and triangles links.
            var counters = new NativeArray<int>(numVertices, allocator);
            Edges = new NativeArray<Edge>(numEdges, allocator);
        ...
        internal unsafe NativeList<Primitive> EnumerateQuadDominantGeometryManaged(NativeArray<int3> triangles, NativeList<float3> vertices)
        {
            int numTriangles = triangles.Length;
            var flags = new NativeArray<PrimitiveFlags>(numTriangles, Allocator.Persistent);
            var quadRoots = new NativeList<Edge>(Allocator.Persistent);
            var triangleRoots = new NativeList<Edge>(Allocator.Persistent);

            // Generate triangle planes
            var planes = new NativeArray<float4>(numTriangles, Allocator.Persistent);
            ...
            var edges = new NativeArray<EdgeData>(Edges.Length, Allocator.Persistent);
            ...
            var freeTriangles = new NativeArray<bool>(numTriangles, Allocator.Persistent);
            for (var i = 0; i < freeTriangles.Length; ++i)
            {
                freeTriangles[i] = true;
            }

            var primitives = new NativeList<Primitive>(Allocator.Persistent);
            ...
            freeTriangles.Dispose();
            DisableEdgesOfAdjacentPrimitivesManaged(primitives, triangles, vertices.AsArray(), planes, flags, quadRoots, triangleRoots);
            flags.Dispose();
            quadRoots.Dispose();
            triangleRoots.Dispose();
            planes.Dispose();
            return primitives;
        }
        ...
        private void DisableEdgesOfAdjacentPrimitivesManaged(
            NativeList<Primitive> primitives, NativeArray<int3> triangles, NativeArray<float3> vertices, NativeArray<float4> planes, NativeArray<PrimitiveFlags> flags,
            NativeList<Edge> quadRoots, NativeList<Edge> triangleRoots)
        {
            var outerBoundary = new NativeArray<Edge>(4, Allocator.Persistent);
            ...
            outerBoundary = new NativeArray<Edge>(3, Allocator.Persistent);
