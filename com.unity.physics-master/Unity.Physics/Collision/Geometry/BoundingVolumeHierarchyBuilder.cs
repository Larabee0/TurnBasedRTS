...
namespace Unity.Physics
{
    ...
    internal partial struct BoundingVolumeHierarchy
    {
        ...
        public unsafe struct Builder
        {
            ...
            public void Dispose()
            {
                ScratchScores.Dispose();
                ScratchPointsX.Dispose();
                ScratchPointsY.Dispose();
                ScratchPointsZ.Dispose();
            }

            void SegregateSah3Managed(Range range, int minItems, ref Range lRange, ref Range rRange)
            {
                if (!ScratchScores.IsCreated)
                {
                    ScratchScores = new NativeArray<float>(Aabbs.Length, Allocator.Persistent);
                    ScratchPointsX = new NativeArray<float4>(Aabbs.Length, Allocator.Persistent);
                    ScratchPointsY = new NativeArray<float4>(Aabbs.Length, Allocator.Persistent);
                    ScratchPointsZ = new NativeArray<float4>(Aabbs.Length, Allocator.Persistent);
                }
            ...
            public void ProcessLargeRangeManaged(Range range, Range* subRanges)
            {
                if (!UseSah)
                {
                    ComputeAxisAndPivot(ref range, out int axis, out float pivot);
                ...
                {
                    Range* temps = stackalloc Range[2];
                    SegregateSah3Managed(range, 2, ref temps[0], ref temps[1]);

                    SegregateSah3Managed(temps[0], 1, ref subRanges[0], ref subRanges[1]);
                    SegregateSah3Managed(temps[1], 1, ref subRanges[2], ref subRanges[3]);
                }
            }
            ...
            public void BuildManaged(Range baseRange)
            {
                Range* ranges = stackalloc Range[Constants.UnaryStackSize];
                int rangeStackSize = 1;
                ranges[0] = baseRange;
                ...
                if (baseRange.Length > 4)
                {
                    do
                    {
                        ...
                        {
                            ProcessLargeRangeManaged(range, subRanges);
                            CreateChildren(subRanges, 4, range.Root, ref FreeNodeIndex, ranges, ref rangeStackSize);
                        }
                    }
                    while (rangeStackSize > 0);
        ...
        public unsafe void BuildManaged(NativeArray<PointAndIndex> points, NativeArray<Aabb> aabbs, out int nodeCount, bool useSah = false)
        {
            m_Nodes[0] = Node.Empty;
            ...
            if (aabbs.Length > 0)
            {
                ...
                Aabb aabb = new Aabb();
                SetAabbFromPoints(ref aabb, (float4*)points.GetUnsafePtr(), points.Length);
                builder.BuildManaged(new Builder.Range(0, points.Length, 1, aabb));
                nodeCount = builder.FreeNodeIndex;
