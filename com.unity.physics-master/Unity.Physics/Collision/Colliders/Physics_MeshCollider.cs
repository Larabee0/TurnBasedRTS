...
namespace Unity.Physics
{
    ...
    public struct MeshCollider : ICompositeCollider
    {
        ...
        public static BlobAssetReference<Collider> CreateManaged(NativeArray<float3> vertices, NativeArray<int3> triangles) =>
            CreateInternalManaged(vertices, triangles, CollisionFilter.Default, Material.Default);
        ...
        public static BlobAssetReference<Collider> CreateManaged(NativeArray<float3> vertices, NativeArray<int3> triangles, CollisionFilter filter) =>
            CreateInternalManaged(vertices, triangles, filter, Material.Default);
        ...
        public static BlobAssetReference<Collider> CreateManaged(NativeArray<float3> vertices, NativeArray<int3> triangles, CollisionFilter filter, Material material) =>
            CreateInternalManaged(vertices, triangles, filter, material);
        ...
        internal static BlobAssetReference<Collider> CreateInternal(NativeArray<float3> vertices, NativeArray<int3> triangles, CollisionFilter filter, Material material, uint forceUniqueBlobID = 0)
        {
            unsafe
            {
                ...
                var tempIndices = new NativeArray<int3>(triangles, Allocator.Temp);
                ...
                var connectivity = new MeshConnectivityBuilder(tempIndices, uniqueVertices.AsArray());
        ...
        internal static BlobAssetReference<Collider> CreateInternalManaged(NativeArray<float3> vertices, NativeArray<int3> triangles, CollisionFilter filter, Material material, uint forceUniqueBlobID = 0)
        {
            unsafe
            {
                SafetyChecks.CheckTriangleIndicesInRangeAndThrow(triangles, vertices.Length, nameof(triangles));
                ...
                // Copy vertices
                var tempVertices = new NativeArray<float3>(vertices, Allocator.Persistent);

                // Triangle indices - needed for WeldVertices
                var tempIndices = new NativeArray<int3>(triangles, Allocator.Persistent);

                // Build connectivity and primitives

                NativeList<float3> uniqueVertices = MeshConnectivityBuilder.WeldVerticesManaged(tempIndices, tempVertices);


                var connectivity = new MeshConnectivityBuilder(tempIndices, uniqueVertices.AsArray(),Allocator.Persistent);
                NativeList<MeshConnectivityBuilder.Primitive> primitives = connectivity.EnumerateQuadDominantGeometryManaged(tempIndices, uniqueVertices);
                uniqueVertices.Dispose();
                // Build bounding volume hierarchy
                int nodeCount = math.max(primitives.Length * 2 + 1, 2); // We need at least two nodes - an "invalid" node and a root node.
                var nodes = new NativeArray<BoundingVolumeHierarchy.Node>(nodeCount, Allocator.Persistent);
                int numNodes = 0;

                {
                    // Prepare data for BVH
                    var points = new NativeList<BoundingVolumeHierarchy.PointAndIndex>(primitives.Length, Allocator.Persistent);
                    var aabbs = new NativeArray<Aabb>(primitives.Length, Allocator.Persistent);
                    ...
                    bvh.BuildManaged(points.AsArray(), aabbs, out numNodes, useSah: true);
                    points.Dispose();
                    aabbs.Dispose();
                }
                ...
                // Build mesh sections
                BoundingVolumeHierarchy.Node* nodesPtr = (BoundingVolumeHierarchy.Node*)nodes.GetUnsafePtr();
                MeshBuilder.TempSection sections = MeshBuilder.BuildSectionsManaged(nodesPtr, numNodes, primitives);

                // Allocate collider
                int meshDataSize = Mesh.CalculateMeshDataSize(numNodes, sections.Ranges);
                int totalColliderSize = Math.NextMultipleOf(sizeof(MeshCollider), 16) + meshDataSize;

                MeshCollider* meshCollider = (MeshCollider*)UnsafeUtility.Malloc(totalColliderSize, 16, Allocator.Persistent);
                ...
                // Copy collider into blob
                var blob = BlobAssetReference<Collider>.Create(meshCollider, totalColliderSize);
                UnsafeUtility.Free(meshCollider, Allocator.Persistent);
                sections.Dispose();
                tempIndices.Dispose();
                tempVertices.Dispose();
                nodes.Dispose();
                primitives.Dispose();
                connectivity.Vertices.Dispose();
                connectivity.Edges.Dispose();
                connectivity.Triangles.Dispose();
                return blob;
            }
        }
